...
class Player(pygame.sprite.Sprite):
    def __init__(self, coordinates, player, character):
        ...
        #### Keys manager ####
        self.key_right, self.key_left, self.key_up, self.key_down, self.key_attack, self.key_jump, self.key_shield, self.key_grab, self.key_special = (False, False, False, False, False, False, False, False, False)
        self.last_key_right, self.last_key_left, self.last_key_up, self.last_key_down, self.last_key_attack, self.last_key_jump, self.last_key_shield, self.last_key_grab, self.last_key_special = (False, False, False, False, False, False, False, False, False)
        self.pressed_key_right, self.pressed_key_left, self.pressed_key_up, self.pressed_key_down, self.pressed_key_attack, self.pressed_key_jump, self.pressed_key_shield, self.pressed_key_grab, self.pressed_key_special = (False, False, False, False, False, False, False, False, False)
        ...
        self.falling_speed, self.max_falling_speed, self.fast_falling_speed = falling_parameters[character]
        self.max_jumps = jumps[character] ## Number of jumps
        self.jump_left = self.max_jumps

        self.stock = global_stock_number
        self.stock_image = pygame.image.load(f'icon/stock/stock_{character}.png')

        if self.character != 'The_Cube':
            self.hitbox = image_var[character]['hitbox']
            self.hitbox_position = image_var[character]['offset']
            self.x_hitbox, self.y_hitbox = tuple(add([self.x, self.y], self.hitbox_position))
            self.bottom = (self.x + self.hitbox_position[0], self.y + self.hitbox_position[1] + self.hitbox[1])
        ...
        self.image_shield = pygame.transform.scale(image_shield, (self.shield_max_size,self.shield_max_size))
        ...
        #### Offscreen image ####
        self.cursor = pygame.image.load(f'sprites/offscreen_cursor{player}.png').convert_alpha()
    ...
    def check_attack(self):
        if self.pressed_key_attack and self.can_act and self.free_fall == False and self.is_attacking == False and self.is_shielding == False and self.doing_special == False and self.grabbing_ledge == False and self.is_grabbing == False and self.is_airdodging == False:
            ...
            self.startup, self.active_frames, self.end_lag = (var[3], var[4], var[5])
            attacks.add(Attack(var[0], var[1], var[2], var[3], var[4], var[6], var[7], self, attack_name, var[8]))
            ...
            if (type(attack).__name__ == 'Attack' or type(attack).__name__ == 'Electric_Ball') and (self.immune_to_attack != (attack.name, attack.player.player) or self.immune_to_attack == (attack.name, attack.player.player) and self.next_hit == 0) and self.player != attack.player.player and pygame.Rect(get_real_pos(self), self.hitbox).colliderect(attack.rect):
                if self.is_shielding:
                    ...
                    if self.shield_size > self.shield_max_size/3:
                        ...
                        pygame.mixer.Sound('Sound_effects/sound_shield_hit.ogg').play()
                    else:
                        attack.kill()
                    ...
                    self.xspeed = calcul_knockback(attack.x_eject, self.percent, self.weight)
                    self.yspeed = calcul_knockback(attack.y_eject, self.percent, self.weight)
                    if abs(self.xspeed) < 1:
                        self.xspeed = sign(self.xspeed)
    ...
    def check_shield(self):
        ...
        if self.is_shielding:
            ...
            if self.shield_size < self.shield_max_size/3:
                pygame.mixer.Sound('Sound_effects/sound_shield_break.ogg').play()
                self.shield_size = self.shield_max_size
                self.hitstun = 120
                self.yspeed = -6
                self.is_shielding = False
    ...
    def check_grab(self):
        if self.grabbed[0] and self.launched == False:
            ...
            self.y = other_y + other.hitbox[1] - self.hitbox[1] - self.hitbox_position[1]
        ...
        if self.grab_player[0] and self.lag == 0:
            if self.key_left or self.key_right or self.key_up:
                ...
                (xspeed, yspeed), hitstun, damage = attack_list[self.character][throw]
                ...
                other.xspeed = calcul_knockback(xspeed, other.percent, other.weight)
                if self.pressed_key_left:
                    other.xspeed *= -1
                other.yspeed = calcul_knockback(yspeed, other.percent, other.weight)
        ...
        if self.pressed_key_grab and self.can_act and self.free_fall == False and self.is_attacking == False and self.is_grabbing == False and self.grabbing_ledge == False and self.is_shielding == False and self.doing_special == False:
            ...
            self.startup, self.active_frames, self.end_lag = (var[2], var[3], var[4])
    ...
    def check_airdodge(self):
        if self.pressed_key_shield and self.on_wall == False and self.can_act and self.free_fall == False and self.is_attacking == False and self.is_shielding == False and self.doing_special == False and self.is_airdodging == False and self.grabbing_ledge == False:
            ...
            self.invincibility_duration = int(global_airdodge_inv_time + 5 * (self.air_speed)**0.5)
            self.invincibility_frame = 0
            self.is_invincible = True
        ...
        if self.is_airdodging:
            if self.invincibility_frame == self.invincibility_duration or self.on_wall:
                ...
                if self.on_wall:
                    self.lag = int(global_airdodge_landing_lag/(1 + 0.2*abs(self.yspeed) + 0.2*abs(self.xspeed)))
                    self.jump_left = self.max_jumps
                    self.apply_gravity = False
                    self.yspeed = 0
                    self.walljump = True
    ...
    def check_ledge(self):
                ...
                for ledge in ledges:
                    if rect.colliderect(ledge.rect) and rect.top > ledge.y and ledge.is_available:
                        ...
                        if self.is_facing_right:
                            self.x = ledge.x + global_ledge_hitbox[0] - self.hitbox[0] - self.hitbox_position[0]
                        else:
                            self.x = ledge.x - hitbox_dependency[self.character] - self.hitbox_position[0]
                        self.y = ledge.y + global_ledge_hitbox[1] - self.hitbox_position[1]
                        ...
                        self.invincibility_frame = 0
                        self.is_invincible = True
                        self.invincibility_duration = calcul_ledge_invincibility(self)
        ...
        if self.neutral_getup:
            ...
            self.invincibility_frame = 0
            self.is_invincible = True
            self.invincibility_duration = int((self.hitbox[1] + global_ledge_hitbox[1])/10) + int(self.hitbox[0]/10)
        ...
        if self.ledgeroll:
            ...
            self.invincibility_frame = 0
            self.is_invincible = True
            self.invincibility_duration = int((self.hitbox[1] + global_ledge_hitbox[1])/5) + 16
    ...
    def offscreen_cursor(self):
        ...
        if real_x + self.hitbox[0] < -16:
            ...
            self.draw_cursor = True
            self.x_cursor, self.y_cursor = (window_size[0] - self.cursor.get_width(), real_y)
        
        elif real_y > window_size[1] + 16:
            self.draw_cursor = True
            self.x_cursor, self.y_cursor = (real_x - self.cursor.get_width()/2 + self.hitbox[0]/2, window_size[1] - self.cursor.get_height())
        
        elif real_y + self.hitbox[1] < -16:
            self.draw_cursor = True
            self.x_cursor, self.y_cursor = (real_x - self.cursor.get_width()/2 + self.hitbox[0]/2, 0)
        ...
        if self.draw_cursor:
            ...
            image_cursor = pygame.transform.scale(self.image, (self.image.get_width()*ratio, self.image.get_height()*ratio))
            image_cursor_x = self.x_cursor + self.cursor.get_width()/2 - image_cursor.get_width()/2
            image_cursor_y = self.y_cursor + self.cursor.get_height()/2 - image_cursor.get_height()/2
    ...
    def draw(self):
        ...
        if self.is_shielding and self.is_rolling == False:
            ...
            shield = pygame.transform.scale(self.image_shield,(self.image_shield.get_width()*scale,self.image_shield.get_height()*scale))
            screen.blit(shield,(self.x_hitbox+self.hitbox[0]/2 - shield.get_width()/2,self.y_hitbox+self.hitbox[1]/2 - shield.get_height()/2))

        if show_hitbox >= 2:
            pygame.draw.rect(screen, (0,255,0), pygame.Rect(get_real_pos(self),self.hitbox))
